...
### Exercise 2 (Optional): Scale Up and Down

1. Scale cassandra cluster Up. Observe in what order kubernetes deploys pods.
    ```
    kubectl scale --replicas=4 statefulset/cassandra
    ```
    1. Check `persistenVolumeClaims` and `persistentVolumes`
    ```
    kubectl get pvc
    kubectl get pv
    ```
1. Scale cassandra cluster Down. Observe in what order kubernetes deletes pods.
     ```
...
```shell
~/environment/kubernetes-training/starting-points/cleanup.sh statefulsets
```
